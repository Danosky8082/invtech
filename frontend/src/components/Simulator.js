...
const Simulator = ({ asset }) => {
  ...
  // currencySymbols kept for reference, but backend provides amountSymbol
    ...
    <div>
      ...
      {result && (
        <div className="result-card">
          ...
          <p>Expected profit: {result.amountSymbol}{result.expectedProfit.toLocaleString(undefined, { minimumFractionDigits: 2 })} {result.amountCurrency}</p>
          <p>Total return: {result.amountSymbol}{result.totalReturn.toLocaleString(undefined, { minimumFractionDigits: 2 })} {result.amountCurrency}</p>
                ...
                {result.projections.map(p => (
                  <div key={p.years} className="projection-card">
                    ...
                    <div className={`projection-profit ${p.profit >= 0 ? 'positive' : 'negative'}`}>
                      {p.profit >= 0 ? '+' : ''}{result.amountSymbol}{p.profit.toFixed(2)} 
                      ({p.percentageReturn}%)
                    </div>
                  </div>
                ))}
              </div>
              <div className="disclaimer">⚠️ This is a simulation based on historical expected returns. Actual results will vary due to market volatility, fees, and taxes.</div>
            </div>
          )}
